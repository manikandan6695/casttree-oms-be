import {
  ...
  import { DatabaseBaseRepositoryAbstract } from "../../base/database.base-repository.abstract";
  ...
  > extends DatabaseBaseRepositoryAbstract<EntityDocument> {
    ...
    async findAll<T = EntityDocument>(
      ...
      if (options?.countDocuments) {
        findAll.countDocuments(options.countDocuments);
      }
    ...
    async create<Dto = any>(
      ...
      return created[0] as unknown as EntityDocument;
    }
    ...
    async updateOne<Dto>(
      ...
      try {
        ...
        return updateData?.acknowledged && updateData?.modifiedCount > 0;
      } catch (err: unknown) {
        throw err;
      }
    }
